...
public class LogPatternAnalysisTool implements Tool {
    ...
    private static final String DEFAULT_DESCRIPTION = "This is a tool used to detect abnormal log patterns by the patterns command in PPL or to detect abnormal log sequences by the log clustering algorithm.";
    ...
    public <T> void run(Map<String, String> parameters, ActionListener<T> listener) {
        ...
        if (Strings.isEmpty(index) || Strings.isEmpty(logFieldName) || Strings.isEmpty(traceFieldName) || Strings.isEmpty(
            normalTimeRangeStart) || Strings.isEmpty(normalTimeRangeEnd) || Strings.isEmpty(abnormalTimeRangeStart) || Strings.isEmpty(
            abnormalTimeRangeEnd)) {
            throw new IllegalArgumentException("Invalid parameters, please check the parameters of LogPatternAnalysisTool,"
        ...
        String normalTimeRangeLogPatternPPL =
            "source=" + index + "| where traceId!='' | where time>'" + normalTimeRangeStart + "' and time <'" + normalTimeRangeEnd
                + "' | patterns " + logFieldName + " method=brain variable_count_threshold=2 | fields " + traceFieldName
                + ", patterns_field";
        executePPL(
            normalTimeRangeLogPatternPPL, ActionListener.wrap(
                normalTimeRangeResult -> {
                    Map<String, Object> normalTimeRangePPLResult =
                        gson.fromJson(normalTimeRangeResult, new TypeToken<Map<String, Object>>() {}.getType());
                    List<List<Object>> normalTimeRangeDataRows =
                        (List<List<Object>>) normalTimeRangePPLResult.getOrDefault("datarows", new ArrayList<>());
                    ...
                    for (List<Object> row : normalTimeRangeDataRows) {
                        ...
                        normalTimeRangePatternCountMap.put(simplifiedPattern, normalTimeRangePatternCountMap.getOrDefault(simplifiedPattern, 0) + 1);
                    }
                    ...
                    String abnormalTimeRangeLogPatternPPL =
                        "source=" + index + "| where traceId!='' | where time>'" + abnormalTimeRangeStart + "' and time <'"
                            + abnormalTimeRangeEnd + "' | patterns " + logFieldName + " method=brain variable_count_threshold=2 | fields "
                            + traceFieldName + ", patterns_field";
                    ...
                    executePPL(
                        abnormalTimeRangeLogPatternPPL, ActionListener.wrap(
                            abnormalTimeRangeResult -> {
                                Map<String, Object> abnormalTimeRangePPLResult =
                                    gson.fromJson(abnormalTimeRangeResult, new TypeToken<Map<String, Object>>() {}.getType());
                                List<List<Object>> abnormalTimeRangeDataRows =
                                    (List<List<Object>>) abnormalTimeRangePPLResult.getOrDefault("datarows", new ArrayList<>());
                                for (List<Object> row : abnormalTimeRangeDataRows) {
                                    ...
                                    abnormalTimeRangePatternCountMap.put(
                                        simplifiedPattern,
                                        abnormalTimeRangePatternCountMap.getOrDefault(simplifiedPattern, 0) + 1
                                    );
                                }
                                ...
                                // Step 3. construct pattern index, take all patterns into consideration, and associate each pattern with a number as the index in the vector
                                ...
                                for (Map.Entry<String, Set<String>> entry : abnormalTimeRangeTraceIdPatternMap.entrySet()) {
                                    ...
                                    for (String pattern : entry.getValue()) {
                                        ...
                                        vector[patternIndex.get(pattern)] =
                                            0.5 * abnormalTimeRangePatternValue.get(pattern) + 0.5 * existenceWeight;
                                    }
                                ...
                                Set<String> abnormalTraces =
                                    findAbnormalTraces(centroids, normalTimeRangeVectorMap, abnormalTimeRangeVectorMap);
                                ...
                                try {
                                    listener.onResponse((T) AccessController.doPrivileged((PrivilegedExceptionAction<String>) () -> gson.toJson(
                                        abnormalClustersCentroids)));
        ...
        while (i < tokens.size()) {
            ...
            for (int seqLen = 1; seqLen <= maxPossibleSeqLen; seqLen++) {
                ...
                while (true) {
                    ...
                    if (start2 + seqLen > tokens.size()) break;
    ...
    private void executePPL(String ppl, ActionListener<String> listener) {
        ...
        client.execute(
            PPLQueryAction.INSTANCE, transportPPLQueryRequest, getPPLTransportActionListener(ActionListener.wrap(
                transportPPLQueryResponse -> listener.onResponse(transportPPLQueryResponse.getResult()), e -> {
    ...
    private List<String> findCentroids(Map<String, double[]> logVectors, List<List<String>> clusters) {
        ...
        for (List<String> cluster : clusters) {
            ...
            String centroid = scores.keySet().iterator().next();
    ...
    private List<List<String>> clusterLogVectors(Map<String, double[]> logVectors) {
        ...
        for (Map.Entry<String, double[]> entry : logVectors.entrySet()) {
            ...
            for (List<String> cluster : clusters) {
                if (cluster.stream().anyMatch(member -> cosineSimilarity(vector, logVectors.get(member)) >= LOG_VECTORS_CLUSTERING_THRESHOLD)) {
                    cluster.add(traceId);
                    placed = true;
                    break;
                }
