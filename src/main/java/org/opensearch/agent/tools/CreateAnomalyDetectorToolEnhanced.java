...
public class CreateAnomalyDetectorToolEnhanced implements WithModelTool {
    ...
    public CreateAnomalyDetectorToolEnhanced(
        ...
        this.customResultIndex = (!Strings.isNullOrEmpty(customResultIndex)) ? customResultIndex : DEFAULT_CUSTOM_RESULT_INDEX;
    ...
    public <T> void run(Map<String, String> parameters, ActionListener<T> listener) {
        ...
        try {
            ...
            int maxRetries = Math.min(Integer.parseInt(parameters.getOrDefault("maxRetries", String.valueOf(MAX_FORMAT_FIX_RETRIES))), 3);
        ...
        for (String idx : indices) {
            ...
            if (idx.length() > 255 || idx.contains("\n") || idx.contains("\r")) {
                throw new IllegalArgumentException(
                    "Invalid index name: " + idx.substring(0, Math.min(50, idx.length())).replaceAll("[\\n\\r]", "")
                );
            }
        }
    }
    ...
    private void processSingleIndex(String indexName, String tenantId, int maxRetries, ActionListener<String> listener) {
        ...
        getIndexInsight(indexName, tenantId, ActionListener.wrap(indexInsight -> {
            getMappingsAndFilterFields(indexName, ActionListener.wrap(mappingContext -> {
            ...
            getMappingsAndFilterFields(
                indexName,
                ActionListener
                    .wrap(mappingContext -> filterNullFieldsIfNeeded(mappingContext, tenantId, maxRetries, listener), listener::onFailure)
            );
        }));
    }
    ...
    private <T> void filterNullFieldsIfNeeded(MappingContext ctx, String tenantId, int maxRetries, ActionListener<T> listener) {
        ...
        log
            .info(
                "Index '{}' has {} fields (>{} threshold), running null field filter",
                ctx.indexName,
                ctx.filteredMapping.size(),
                FIELD_FILTER_THRESHOLD
            );

        filterNullFields(ctx.indexName, ctx.filteredMapping, ActionListener.wrap(filtered -> {
            log.info("Null filter reduced '{}' from {} to {} fields", ctx.indexName, ctx.filteredMapping.size(), filtered.size());
            MappingContext filteredCtx = new MappingContext(
                ctx.indexName,
                filtered,
                ctx.dateFields,
                ctx.indexInsight,
                ctx.detectorDecisions,
                ctx.sampleDocs,
                ctx.dataDensity24h
            );
    ...
    private <T> void proceedWithLLM(MappingContext mappingContext, String tenantId, int maxRetries, ActionListener<T> listener) {
        ...
        filterDateFieldsByDensity(
            mappingContext,
            ActionListener.wrap(filteredCtx -> enrichAndCreateDetectors(filteredCtx, tenantId, maxRetries, listener), e -> {
    ...
    private <T> void enrichAndCreateDetectors(MappingContext ctx, String tenantId, int maxRetries, ActionListener<T> listener) {
        ...
        }, e -> { createMultipleDetectors(ctx, tenantId, maxRetries, new ArrayList<>(), new ArrayList<>(), 0, listener); }));
    }
    ...
    private static String resolveKeywordField(Map<String, String> fields, String name) {
        if (fields.containsKey(name) && "keyword".equals(fields.get(name)))
            return name;
        if (fields.containsKey(name + ".keyword"))
            return name + ".keyword";
    ...
    private static class OtelDetectorSpec {
        ...
        OtelDetectorSpec(String nameSuffix, String timeField, String categoryField, String featureField, QueryBuilder featureFilter) {
    ...
    private List<OtelDetectorSpec> buildOtelSpecs(OtelSignalType type, Map<String, String> fields) {
        ...
        if (type == OtelSignalType.TRACES) {
            specs
                .add(
                    new OtelDetectorSpec(
                        "trace-errors",
                        timeField,
                        resolveKeywordField(fields, "serviceName"),
                        timeField,
                        QueryBuilders.termQuery("status.code", 2)
                    )
                );
            specs.add(new OtelDetectorSpec("trace-throughput", timeField, resolveKeywordField(fields, "serviceName"), timeField, null));
        } else {
            specs
                .add(
                    new OtelDetectorSpec(
                        "log-errors",
                        timeField,
                        resolveKeywordField(fields, "resource.attributes.service.name"),
                        timeField,
    ...
    private <T> void createOtelDetectorSequentially(
        ...
        if (idx >= specs.size()) {
            log
                .info(
                    "OTel detectors done for '{}': {}/{} succeeded",
                    indexName,
        ...
        AggregationBuilder featureAgg = spec.featureFilter != null
            ? AggregationBuilders.filter(spec.nameSuffix.replace("-", "_"), spec.featureFilter).subAggregation(innerAgg)
            : innerAgg;
        ...
        AnomalyDetector detector = new AnomalyDetector(
            ...
            null,
            null,
            new IntervalTimeConfiguration(calculateFrequencyMinutes(DEFAULT_OTEL_INTERVAL_MINUTES), ChronoUnit.MINUTES),
            true
        );
    ...
    private <T> void suggestAndCreateOtelDetector(
        ...
        adClient.suggestAnomalyDetector(suggestRequest, ActionListener.wrap(suggestResp -> {
            AnomalyDetector optimized = suggestResp.getInterval() != null ? applySuggestionsToDetector(detector, suggestResp) : detector;
    ...
    private <T> void doCreateAndStartOtelDetector(
        ...
        adClient.createAnomalyDetector(createReq, ActionListener.wrap(createResp -> {
            String detectorId = createResp.getId();
            log.info("OTel detector created: {} ({})", detectorName, detectorId);
            JobRequest startReq = new JobRequest(
                detectorId,
            ...
            adClient.startAnomalyDetector(startReq, ActionListener.wrap(startResp -> {
                results
                    .add(
                        DetectorResult
    ...
    private void filterNullFields(String indexName, Map<String, String> mapping, ActionListener<Map<String, String>> listener) {
        ...
        for (int i = 0; i < fieldOrder.size(); i++) {
            filters.addAggregator(AggregationBuilders.filter("f_" + i, QueryBuilders.existsQuery(fieldOrder.get(i))));
        }
        SamplerAggregationBuilder sampler = AggregationBuilders
            .sampler("sample")
            .shardSize(FIELD_FILTER_SAMPLE_SIZE)
            .subAggregations(filters);
    ...
    private void getSampleDocuments(String indexName, int size, ActionListener<String> listener) {
        SearchRequest request = new SearchRequest(indexName)
            .source(new SearchSourceBuilder().size(size).query(QueryBuilders.matchAllQuery()).sort("_doc").trackTotalHits(false));
        client.search(request, ActionListener.wrap(response -> {
    ...
    private void getDataDensity(String indexName, String dateField, ActionListener<Long> listener) {
        SearchRequest request = new SearchRequest(indexName)
            .source(new SearchSourceBuilder().size(0).query(QueryBuilders.rangeQuery(dateField).gte("now-24h")).trackTotalHits(true));
        client.search(request, ActionListener.wrap(response -> listener.onResponse(response.getHits().getTotalHits().value()), e -> {
    ...
    private void checkNextDateField(
        MappingContext ctx,
        List<String> dateFieldList,
        int idx,
        Map<String, Long> counts,
        ActionListener<MappingContext> listener
    ) {
        if (idx >= dateFieldList.size()) {
            // All fields checked — filter to those with data
            Set<String> viable = dateFieldList
                .stream()
    ...
    private String constructPrompt(
        ...
        if (fields.size() > MAX_PROMPT_FIELDS) {
            ...
            fields = fields.entrySet().stream().limit(MAX_PROMPT_FIELDS).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        }
    ...
    private static int calculateFrequencyMinutes(int intervalMinutes) {
        ...
        if (minFreq >= MAX_FREQUENCY_MINUTES)
            return MAX_FREQUENCY_MINUTES;
        int multiples = (MAX_FREQUENCY_MINUTES - minFreq) / intervalMinutes;
        return minFreq + Randomness.get().nextInt(multiples + 1) * intervalMinutes;
    }
    ...
    private AnomalyDetector buildAnomalyDetectorFromSuggestions(Map<String, String> suggestions, QueryBuilder filterQuery) {
        ...
        boolean isHC = !categoryField.isEmpty() && !categoryField.equalsIgnoreCase("null") && !categoryField.equalsIgnoreCase("none");
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            cleanField = cleanField.replace("${dateFields}", actualDateField).replace("${indexInfo.dateFields}", actualDateField);
            ...
            if (featureFilter != null && isHC && !filterAppliedInFeature && "count".equalsIgnoreCase(method)) {
                featureAgg = AggregationBuilders
                    .filter("feature_" + cleanField + "_" + method + "_filter", featureFilter)
        ...
        if (isHC) {
            categoryFields = java.util.Arrays
                .stream(categoryField.split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
        ...
        QueryBuilder detectorFilter = (featureFilter != null && !filterAppliedInFeature) ? featureFilter : QueryBuilders.matchAllQuery();
        ...
        if (description.isEmpty())
            description = DEFAULT_DETECTOR_DESCRIPTION;
    ...
    private <T> void parseAndRetryWithLLM(
        ...
        if (parsed == null) {
            ...
            if (currentRetry < maxRetries) {
                String parseError = "Cannot parse response format. Expected: " + LLM_OUTPUT_FORMAT;
                retryWithFormatFix(
                    parseError,
                    indexName,
                    dateFields,
    ...
    private <T> void validateDetectorPhase(
        ...
        if (currentRetry >= MAX_DETECTOR_VALIDATION_RETRIES) {
            listener.onFailure(new RuntimeException("Detector validation failed after " + MAX_DETECTOR_VALIDATION_RETRIES + " retries"));
            return;
        }
        try {
    ...
    private <T> void retryDetectorValidation(
        ...
        log
            .info(
                "LLM_FIX_PROMPT,index={},retry={},error={}",
                updatedCtx.indexName,
                currentRetry,
    ...
    private static String buildRetryContext(MappingContext ctx) {
        ...
        if (ctx.sampleDocs != null) {
            String truncated = ctx.sampleDocs.length() > 1000 ? ctx.sampleDocs.substring(0, 1000) + "..." : ctx.sampleDocs;
    ...
    private <T> void suggestHyperParametersPhase(
        AnomalyDetector detector,
        MappingContext mappingContext,
        String tenantId,
        int maxRetries,
            ...
            public void onResponse(ValidateConfigResponse response) {
                ...
                if (response.getIssue() != null) {
                    ...
                    if (intervalSuggestion != null && currentRetry < MAX_MODEL_VALIDATION_RETRIES) {
                        ...
                        log
                            .info(
                                "Applying validation suggestion: {}={} for '{}'",
                                issueType,
                                intervalSuggestion,
                                getDetectorIndex(detector)
                            );
    ...
    private <T> void retryModelValidation(
        ...
        Map<String, String> currentSuggestions = Map
            .of(
                ...
                OUTPUT_KEY_CATEGORY_FIELD,
                detector.getCategoryFields() == null || detector.getCategoryFields().isEmpty()
                    ? ""
                    : String.join(",", detector.getCategoryFields()),
                OUTPUT_KEY_AGGREGATION_FIELD,
    ...
    private static final Set<String> KNOWN_LLM_KEYS = Set
        .of("category_field", "aggregation_field", "aggregation_method", "filter", "interval", "date_field");
    ...
    private static Map<String, String> parseLLMResponse(String llmResponse) {
        ...
        while (braces.find()) {
            ...
            if (!content.contains("category_field"))
                continue;
    ...
    private static String buildAttemptSummary(Map<String, String> suggestions, String error) {
        return "category="
            + suggestions.getOrDefault(OUTPUT_KEY_CATEGORY_FIELD, "")
            + ", field="
            + suggestions.getOrDefault(OUTPUT_KEY_AGGREGATION_FIELD, "")
            + ":"
            + suggestions.getOrDefault(OUTPUT_KEY_AGGREGATION_METHOD, "")
            + ", interval="
            + suggestions.getOrDefault("interval", "?")
            + " → "
    ...
    private static class MappingContext {
        ...
        MappingContext(
            String indexName,
            Map<String, String> filteredMapping,
            Set<String> dateFields,
            String indexInsight,
            List<String> detectorDecisions,
            String sampleDocs,
            long dataDensity24h
        ) {
    ...
    private <T> void createMultipleDetectors(
        ...
        if (ctx.sampleDocs != null) {
            String truncated = ctx.sampleDocs.length() > 2000 ? ctx.sampleDocs.substring(0, 2000) + "..." : ctx.sampleDocs;
