...
public class CreateAlertToolTests {
  ...
  private void initMLTensors(Map<String, ?> modelReturns) {
    ModelTensor modelTensor = new ModelTensor("tensor", new Number[0], new long[0],
        MLResultDataType.STRING, null, null,
        modelReturns);
  ...
  public void testTool() {
    ...
    tool.run(ImmutableMap.of("indices", mockedIndices, "question", "test_question"),
        ActionListener.<String>wrap(response -> assertEquals(jsonResponse, response), e -> fail("Tool runs failed: " + e.getMessage())));
  ...
  public void testToolWithNoJsonResponse() {
    ...
    Exception exception = assertThrows(IllegalArgumentException.class,
        () -> tool.run(ImmutableMap.of("indices", mockedIndices, "question", "test_question"),
            ActionListener.<String>wrap(response -> assertEquals(noJsonResponse, response), e -> {throw new IllegalArgumentException(e.getMessage());})));
  ...
  public void testToolWithPredictModelFailed() {
    ...
    Exception exception = assertThrows(RuntimeException.class,
        () -> tool.run(ImmutableMap.of("indices", mockedIndices, "question", "test_question"),
            ActionListener.<String>wrap(response -> assertEquals(jsonResponse, response), e -> {throw new RuntimeException(e.getMessage());})));
